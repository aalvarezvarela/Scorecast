...
def test_questionable_players_get_the_same_reduced_weighted_calculation():
    df_team, df_players, df_injuries = _frames()
    target = df_team.GAME_ID.iloc[3]
    out, _ = add_player_history_features(
        ...
        report_questionable_sets={(target, TEAM): [200000, 200001]},
    )
    row = out.loc[out.GAME_ID.eq(target)].iloc[0]
    expected, plain_mean = _expected_weighted(range(2))

    assert row["QUESTIONABLE_WEIGHTED_OFF_RATING_BEFORE"] == pytest.approx(expected)
    assert row["QUESTIONABLE_WEIGHTED_OFF_RATING_BEFORE"] > plain_mean
    assert row["N_QUESTIONABLE_PLAYERS_BEFORE"] == 2
    assert row["N_INJURED_PLAYERS_BEFORE"] == 0
    assert "TOP1_QUESTIONABLE_PLAYER_OFF_RATING_BEFORE" not in out.columns
