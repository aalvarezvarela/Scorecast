...
def _stint(date, game_id, *, special, home_pts):
    return dict(
        game_date=date, game_id=game_id, start_ds=0, end_ds=7200,
        home_lineup=(special, 2, 3, 4, 5), away_lineup=(11, 12, 13, 14, 15),
        home_pts=home_pts, away_pts=20,
        home_fga=20, home_fta=0, home_oreb=0, home_tov=0,
        away_fga=20, away_fta=0, away_oreb=0, away_tov=0,
    )


def test_planted_offense_signal_recovers_positive_difference():
    history = pd.DataFrame([
        _stint(f"2025-01-{day:02d}", f"g{day}", special=(1 if day % 2 else 6),
    ...
    assert ratings.loc["1", "o_rating"] - ratings.loc["6", "o_rating"] == pytest.approx(10, abs=2)
...
def test_incremental_normal_equations_match_batch():
    ...
    np.testing.assert_allclose(decayed.solve(5)[0], batch_decayed.solve(5)[0], atol=1e-8)
