#!/usr/bin/env bash
# Schema-2.5 closing totals: line error then total points, protocols a -> b -> c.
set -uo pipefail

cd "$(dirname "$0")/../.." || exit 1

CAMPAIGN="schema25_closing_2026_09"
PART="part1_totals"
...
CONFIGS=(
  "$CONFIG_DIR/l_a_line_error_12x50_early_stop.yaml"
  "$CONFIG_DIR/l_b_line_error_6x100_early_stop.yaml"
  "$CONFIG_DIR/l_c_line_error_6x100_tuned_rounds.yaml"
  "$CONFIG_DIR/t_a_total_points_12x50_early_stop.yaml"
  "$CONFIG_DIR/t_b_total_points_6x100_early_stop.yaml"
  "$CONFIG_DIR/t_c_total_points_6x100_tuned_rounds.yaml"
)
