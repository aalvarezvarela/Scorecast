#!/usr/bin/env bash
# Schema-2.5 closing-line total_points, always 6x100 test-anchored CV: five cells,
# each changing ONE thing from the reference.
#
#   t_ref       NaN 300, window 4,000, seed 16
#   t_ref_rep   identical, seed 17 -- the noise floor; read it first
#   t_tg6200    window 6,200
#   t_tg2500    window 2,500
#   t_na150     NaN budget 150, window 4,000
#
...
CAMPAIGN="closing_total_points_6x100_2_5_2026_09"
CONFIG_DIR="experiments/${CAMPAIGN}"
LOG_DIR="artifacts/logs/${CAMPAIGN}_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"
LOG="$LOG_DIR/campaign.log"
CONFIGS=(
  "$CONFIG_DIR/t_ref.yaml"
  "$CONFIG_DIR/t_ref_rep.yaml"
  "$CONFIG_DIR/t_tg6200.yaml"
  "$CONFIG_DIR/t_na150.yaml"
  "$CONFIG_DIR/t_tg2500.yaml"
)
