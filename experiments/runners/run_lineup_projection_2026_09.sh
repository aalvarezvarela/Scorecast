#!/usr/bin/env bash
# lineup_projection_2026_09: schema 2_6 with and without the LU_* lineup family,
# for both totals strategies. Runs sequentially on one CUDA device, control
# before treatment within each strategy. Set SKIP_EXISTING=1 to resume safely.
set -uo pipefail

cd "$(dirname "$0")/../.." || exit 1

CAMPAIGN="lineup_projection_2026_09"
...
CONFIGS=(
  "$CONFIG_DIR/a_line_error_control.yaml"
  "$CONFIG_DIR/b_line_error_lineup.yaml"
  "$CONFIG_DIR/c_total_points_control.yaml"
  "$CONFIG_DIR/d_total_points_lineup.yaml"
)
...
log "Lineup projection campaign started $(date)"
log "Runs: ${#CONFIGS[@]} sequential CUDA experiments, 150 trials each"
log "Seeds: 16 + [101, 202, 303]; floor 2021-22; holdout last 90 days"
log "Logs: $LOG_DIR"
