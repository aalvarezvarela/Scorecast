#!/usr/bin/env bash
# Schema-2.5 intermediate total points at T-360, T-720, T-960.
set -uo pipefail

cd "$(dirname "$0")/../.." || exit 1

CAMPAIGN="intermediate_horizons_2_5_2026_09"
PART="part2_total_points"
CONFIG_DIR="experiments/${CAMPAIGN}"
LOG_DIR="artifacts/logs/${CAMPAIGN}_${PART}_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"
LOG="$LOG_DIR/campaign.log"
CONFIGS=(
  "$CONFIG_DIR/t_t360_total_points.yaml"
  "$CONFIG_DIR/t_t720_total_points.yaml"
  "$CONFIG_DIR/t_t960_total_points.yaml"
)
