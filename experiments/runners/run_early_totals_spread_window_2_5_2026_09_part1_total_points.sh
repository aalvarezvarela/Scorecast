#!/usr/bin/env bash
# Schema-2.5 early totals+spread windows, part1 (total_points). Run both parts concurrently.
# Set SKIP_EXISTING=1 to resume completed configs.
set -uo pipefail

cd "$(dirname "$0")/../.." || exit 1

CAMPAIGN="early_totals_spread_window_2_5_2026_09"
PART="part1_total_points"
CONFIG_DIR="experiments/${CAMPAIGN}"
LOG_DIR="artifacts/logs/${CAMPAIGN}_${PART}_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"
LOG="$LOG_DIR/campaign.log"
CONFIGS=(
  "$CONFIG_DIR/t_t0_total_points.yaml"
  "$CONFIG_DIR/t_t60_total_points.yaml"
  "$CONFIG_DIR/t_t180_total_points.yaml"
  "$CONFIG_DIR/t_t300_total_points.yaml"
  "$CONFIG_DIR/t_t420_total_points.yaml"
  "$CONFIG_DIR/t_t540_total_points.yaml"
  "$CONFIG_DIR/t_t660_total_points.yaml"
  "$CONFIG_DIR/t_t840_total_points.yaml"
  "$CONFIG_DIR/t_t1080_total_points.yaml"
)
