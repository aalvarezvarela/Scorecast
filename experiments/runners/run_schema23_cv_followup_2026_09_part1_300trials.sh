#!/usr/bin/env bash
# Schema-2.3 closing line/total searches with a doubled Optuna budget.
set -uo pipefail

cd "$(dirname "$0")/../.." || exit 1

CAMPAIGN="fixed50_optuna_2_3_followup_2026_09"
PART="part1_300trials"
CONFIG_DIR="experiments/${CAMPAIGN}"
LOG_DIR="artifacts/logs/${CAMPAIGN}_${PART}_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"
LOG="$LOG_DIR/campaign.log"
CONFIGS=(
  "$CONFIG_DIR/a_line_error_300_trials_fixed50.yaml"
  "$CONFIG_DIR/b_total_points_300_trials_fixed50.yaml"
)
...
log "Two sequential CUDA runs; 300 trials each; original 12x50 CV; seed 16."
log "Logs: $LOG_DIR"
