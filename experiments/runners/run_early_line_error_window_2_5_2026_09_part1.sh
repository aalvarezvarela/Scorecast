#!/usr/bin/env bash
# Schema-2.5 early line-error windows, part1. Run both parts concurrently.
# Set SKIP_EXISTING=1 to resume completed configs.
set -uo pipefail

cd "$(dirname "$0")/../.." || exit 1

CAMPAIGN="early_line_error_window_2_5_2026_09"
PART="part1"
...
CONFIGS=(
  "$CONFIG_DIR/l_t0_line_error.yaml"
  "$CONFIG_DIR/l_t60_line_error.yaml"
  "$CONFIG_DIR/l_t180_line_error.yaml"
  "$CONFIG_DIR/l_t300_line_error.yaml"
  "$CONFIG_DIR/l_t360_line_error.yaml"
  "$CONFIG_DIR/l_t420_line_error.yaml"
  "$CONFIG_DIR/l_t480_line_error.yaml"
  "$CONFIG_DIR/l_t540_line_error.yaml"
  "$CONFIG_DIR/l_t600_line_error.yaml"
)
